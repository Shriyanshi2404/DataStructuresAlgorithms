 ...
 * 2. Since for last element there is no next greater element, we will add -1 to the list and push the last element of array into stack
...
public class NextGreaterElement {
    ...
    public static ArrayList<Integer> nextLargerElement(int[] arr) {
        ...
        for(int i=arr.length-2; i>=0; i--)
        {
            ...
            // if current element is less than top element of stack
            if(ele < st.peek())
            {
                int top_ele = st.peek();
                list.add(top_ele);
            ...
            // if current element is greater than or equal to top element of stack
            else if(ele >= st.peek())
            {
                // pop all the smaller or equal element from the stack until we find a greater element or the stack is empty
